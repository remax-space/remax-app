...
async function callClaude(prompt, system, maxTokens){
  ...
  // Chave salva no localStorage (configurada pelo usuário)
  var ANTHR_KEY = localStorage.getItem('_remax_anthr_key') || '';
  ...
  // Tentativa 1: Direto na Anthropic (se tiver chave configurada)
  if(ANTHR_KEY && ANTHR_KEY.startsWith('sk-ant-')){
    try{
    ...
    } catch(e1){ console.warn('Direto falhou:', e1.message); }
  }

  // Tentativa 2: Proxy Supabase
  try{
    var resp2 = await fetch(SUPA_URL, {
      method:'POST',
  ...
  } catch(e2){ console.warn('Supabase falhou:', e2.message); }

  // Tentativa 3: corsproxy.io com chave
  if(ANTHR_KEY && ANTHR_KEY.startsWith('sk-ant-')){
    try{
      var resp3 = await fetch('https://corsproxy.io/?https://api.anthropic.com/v1/messages', {
        method:'POST',
        headers:{'Content-Type':'application/json','x-anthropic-version':'2023-06-01','x-api-key':ANTHR_KEY},
        body: body,
        signal: AbortSignal.timeout(15000)
      });
      if(resp3.ok){
    ...
    } catch(e3){ console.warn('corsproxy falhou:', e3.message); }
  }

  // Sem chave configurada
  if(!ANTHR_KEY || !ANTHR_KEY.startsWith('sk-ant-')){
    throw new Error('Chave API não configurada. Vá em Sistema → Configurar IA.');
  }
...
async function callClaudeVision(mensagens, system, maxTokens){
  ...
  if(ANTHR_KEY && ANTHR_KEY.startsWith('sk-ant-')){
    ...
    } catch(e1){ console.warn('Vision direto falhou:', e1.message); }
  }
  ...
  } catch(e2){ console.warn('Vision Supabase falhou:', e2.message); }

  if(!ANTHR_KEY || !ANTHR_KEY.startsWith('sk-ant-')){
    throw new Error('Chave API não configurada. Vá em Sistema → Configurar IA.');
  }
  throw new Error('Análise de imagem indisponível. Verifique sua conexão.');
}
...
function configurarChaveIA(){
  var atual = localStorage.getItem('_remax_anthr_key')||'';
  oM('🔑 Configurar Chave da IA',
    ...
    '🔐 A chave fica salva <strong>somente neste navegador</strong>. Cada computador precisa configurar uma vez.'+
    '</div>'+
    '<div class="fg"><label>Chave API Anthropic (sk-ant-...)</label>'+
    '<input id="cfg-key" type="password" placeholder="sk-ant-api03-..." value="'+( atual?'●'.repeat(20):'')+'">'+
    '</div>'+
    '<div style="font-size:10px;color:#94a3b8;margin-top:6px">'+
    'Obtenha em: console.anthropic.com → API Keys'+
    '</div>',
    function(){
      var k=(document.getElementById('cfg-key')||{}).value||'';
      if(k && !k.startsWith('●') && k.startsWith('sk-ant-')){
        localStorage.setItem('_remax_anthr_key', k);
        alert('✅ Chave configurada com sucesso! A IA já está disponível.');
      } else if(k.startsWith('●')){
        alert('Chave mantida sem alteração.');
      } else {
        localStorage.removeItem('_remax_anthr_key');
